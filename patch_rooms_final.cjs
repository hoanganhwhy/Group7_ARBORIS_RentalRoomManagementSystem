...
// 1. Add getImageUrl import
if (!code.includes('getImageUrl,')) {
  code = code.replace('getRooms,', 'getRooms,\n  getImageUrl,');
}
...
if (!code.includes('setImageFile(null);')) {
  code = code.replace(
    /setFormData\(\{ room_number: '', floor: 1, area_sqm: '', monthly_rent: '', max_occupants: 2, status: 'available', description: '', location: 'Cơ sở chính' \}\);\s*setIsModalOpen\(true\);/s,
  ...
  code = code.replace(
    /setEditingRoom\(room\);\s*setFormData\(\{/s,
    `setEditingRoom(room);\n      setImageFile(null);\n      setRemoveImage(false);\n      setFormData({`
  );
}

// 4. Update handleSubmit
const submitRegex = /const payload = \{\s*\.\.\.formData,\s*area_sqm: parseFloat\(formData\.area_sqm as any\) \|\| 0,\s*monthly_rent: parseFloat\(formData\.monthly_rent as any\) \|\| 0,\s*\};\s*let savedRoom: Room;/s;
