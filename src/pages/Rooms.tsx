  ...
  async function handleGenerateContract(assignment: RoomAssignment) {
    ...
    try {
      if (contractAssignment.file_hop_dong) {
        window.open(`${import.meta.env.VITE_API_URL || '/api'}/contracts/${assignment.id}/download`, '_blank');
        return;
      }
      
      const res = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/contracts/${assignment.id}/generate`, {
        method: 'POST',
        credentials: 'include'
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-10">
      ...
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-serif lining-nums tabular-nums text-charcoal-900 tracking-wide">Quản lý phòng</h1>
