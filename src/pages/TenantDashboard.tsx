...
export function TenantDashboard({ onNavigate }: TenantDashboardProps = {}) {
  ...
  const loadNotifications = async () => {
    try {
      const res = await fetch(`${API_URL}/notifications/my?archive=false&limit=4`, {
        credentials: 'include'
      });
      if (res.ok) {
        const result = await res.json();
        setNotifications(result.data ? result.data.slice(0, 4) : []);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const unpaidInvoices = portalData?.unpaidInvoices || [];
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-wood-100/50 flex flex-col justify-between h-40">
          <div>
            <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-charcoal-500 mb-1">Thông báo</p>
            <p className="text-3xl font-serif text-charcoal-900 tracking-tight">{notifications.filter(n => !n.is_read).length}</p>
            <p className="text-xs text-charcoal-400 mt-1 font-light">Chưa đọc</p>
          </div>
          <div className="flex items-center justify-between text-wood-600">
            <span className="text-xs font-semibold uppercase tracking-wider cursor-pointer hover:text-wood-800" onClick={() => onNavigate?.('notifications')}>Xem tất cả</span>
